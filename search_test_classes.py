...
class GraphSearchTest(test_classes.TestCase):
    ...
    def __init__(self, question, test_dict):
        ...
        self.exact_expansion_order = test_dict.get('exactExpansionOrder', 'True').lower() == "true"
...
class PacmanSearchTest(test_classes.TestCase):
    ...
    def execute(self, grades, module_dict, solution_dict):
        ...
        if expanded > self.leeway_factor * gold_expanded and expanded > gold_expanded + 1:
            ...
            grades.add_message('')
            grades.add_message('\tcorrect nodes expanded: %s (leewayFactor %s)' % (gold_expanded, self.leeway_factor))
            return False
...
class HeuristicTest(test_classes.TestCase):
    ...
    def check_heuristic(self, heuristic, problem, state, solution_cost):
        ...
        for (succ, action, step_cost) in problem.get_successors(state):
            ...
            if h0 - h1 > step_cost: return False, 'Heuristic failed consistency test'
